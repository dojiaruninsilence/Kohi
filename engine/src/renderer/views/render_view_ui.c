...
b8 render_view_ui_on_build_packet(const struct render_view* self, void* data, struct render_view_packet* out_packet) {
    ...
    // obtain all geometries fromm the current scene.
    // iterate all of the meshes and add them to the packet's geometries collection
...
b8 render_view_ui_on_render(const struct render_view* self, const struct render_view_packet* packet, u64 frame_number, u64 render_target_index) {
    ...
    for (u32 p = 0; p < self->renderpass_count; ++p) {
        ...
        for (u32 i = 0; i < count; ++i) {
            ...
            // update the material if it hasn't already been this fram. theis keeps the same materials from being updated multiple
            // times. it still needs to be bound either way, so this check result gets passed to the backend which either
