...
void vulkan_material_shader_set_model(vulkan_context* context, struct vulkan_material_shader* shader, mat4 model) {
    if (context && shader) {                                                                     // verify that there is a context and shader
        u32 image_index = context->image_index;                                                  // copy the image index
        VkCommandBuffer command_buffer = context->graphics_command_buffers[image_index].handle;  // get the command buffer at that index
...
void vulkan_material_shader_release_resources(vulkan_context* context, struct vulkan_material_shader* shader, material* material) {
    vulkan_material_shader_instance_state* instance_states = &shader->instance_states[material->internal_id];
    ...
    VkResult result = vkFreeDescriptorSets(context->device.logical_device, shader->object_descriptor_pool, descriptor_set_count, instance_states->descriptor_sets);
    if (result != VK_SUCCESS) {
    ...
    for (u32 i = 0; i < VULKAN_MATERIAL_SHADER_DESCRIPTOR_COUNT; ++i) {
        for (u32 j = 0; j < 3; ++j) {
            instance_states->descriptor_states[i].generations[j] = INVALID_ID;
            instance_states->descriptor_states[i].ids[j] = INVALID_ID;
        }
    }
