...
b8 create_shader_module(
    ...
    // build the filename, which will also be used as the resource name
    char file_name[512];                                            // create an array of characters 512 long called file name -- buffer for the string format
    string_format(file_name, "shaders/%s.%s.spv", name, type_str);  // use our string format, pass in the name, and type string, and a start string, concat to "assets/shaders/[name value].[type_str value].spv" and stor in filename
