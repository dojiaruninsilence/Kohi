...
b8 vulkan_renderer_backend_initialize(renderer_backend* backend, const renderer_backend_config* config, u8* out_window_render_target_count) {
    ...
    // just set some default values for the framebuffer for now. it doesnt really matter what
    // these are because they will be overridden, but are needed for the swapchain creation
    ...
    app_info.pApplicationName = config->application_name;               // tell vulkan what the name of the application
...
// vulkan debugger
    ...
    // save off the number fo images we have as the number of render targets needed
    *out_window_render_target_count = context.swapchain.image_count;
    ...
    // the renderpass table will be a lookup of array indices. start off with an invalid id
    ...
    for (u32 i = 0; i < config->renderpass_count; ++i) {
        // TODO: move to a function for reusablility
        // make sure there are no collisions with the nane first
        u32 id = INVALID_ID;
...
void vulkan_renderer_backend_on_resized(renderer_backend* backend, u16 width, u16 height) {
    ...
    context.framebuffer_width = width;      // pass in the width
    context.framebuffer_height = height;    // pass in the height
...
b8 recreate_swapchain(renderer_backend* backend) {  // private function to recreate the swapchain, just takes in a pointer to the backend
    ...
    vulkan_swapchain_recreate(
        ...
        context.framebuffer_width,   // the cached frame buffer width
...
void vulkan_renderer_texture_create_writeable(texture* t) {
    ...
    vulkan_image_create(
        ...
        t->type,   // is going to be a 2d image
...
void vulkan_renderpass_create(renderpass* out_renderpass, f32 depth, u32 stencil, b8 has_prev_pass, b8 has_next_pass) {
    ...
    // depth attchment if there is one
    ...
    // renderpass dependencies.  TODO: make this configurable.
