// dont have a linux environment to test on so i just copied from his repo in an attempt to keep the projects as similar as i can.
// widowing is different in linux - this is all in video #005 i am going to watch it but not take too many notes, if i ever get into wanting to support linux i will come back to this
// dont even really understand how linux works, so this was all very confusing
// this file will not collide with the win32 c file, so if it is i fucked it up somewhere along the way

#include "platform.h"

// Linux platform layer.
#if KPLATFORM_LINUX  // check if it is linux, stop here if not
...
// logging and memory allocation stuffs
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
...
// similar to widows but not wuite
...
b8 platform_startup(
    ...
    // Turn off key repeats. - apparently these can get annoying, this is global across the entire os, so make sure to turn them back on or the comp will be fucked til reboot
    ...
    // Create the window -- linux calls windows cookies
    ...
    // Change the title - set the title
    xcb_change_property(
        state->connection,
    ...
    // changing some of the callbacks - he was iffy about the effectivesness of all of this stuff
    xcb_change_property(
        state->connection,
        XCB_PROP_MODE_REPLACE,
        state->window,
    ...
    // Flush the stream -- make sure that anything that is buffered is out put, still unsure on this stuff so look up
