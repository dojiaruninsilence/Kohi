...
b8 material_loader_load(struct resource_loader* self, const char* name, resource* out_resource) {
    ...
    // put all the different stuff together to make a string that is the filepath to the file
...
void material_loader_unload(struct resource_loader* self, resource* resource) {
    ...
    if (path_length) {
        kfree(resource->full_path, sizeof(char) * path_length, MEMORY_TAG_STRING);
    }
