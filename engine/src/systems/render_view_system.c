...
b8 render_view_system_initialize(u64* memory_requirement, void* state, render_view_system_config config) {
    ...
    // block of memory will contain the state structure, then the block for the hashtable, then the block for the array
    ...
    *memory_requirement = struct_requirement + hashtable_requirement + array_requirement;
    ...
    // array block is after the hashtable
...
b8 render_view_system_create(const render_view_config* config) {
    ...
    // make sure that there is not already an entry with this name already registered
    ...
    // TODO: leaking the name, creates a destroy method and kill this
    ...
    // TODO: factory pattern (with register, ect for each type)?
